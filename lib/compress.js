module.exports = ( response, body, accept_encoding, level = 1 ) => {
	...
	if ( encodings.includes( 'br' ) ) {
		...
		return zlib.brotliCompressSync( body, { level: level } );
	} else if ( encodings.includes( 'gzip' ) ) {
		response.writeHead( 200, { 'Content-Encoding': 'gzip' } );
		return zlib.gzipSync( body, { level: level } );
	} else if ( encodings.includes( 'deflate' ) ) {
		response.writeHead( 200, { 'Content-Encoding': 'deflate' } );
		return zlib.deflateSync( body, { level: level } );
	} else {
